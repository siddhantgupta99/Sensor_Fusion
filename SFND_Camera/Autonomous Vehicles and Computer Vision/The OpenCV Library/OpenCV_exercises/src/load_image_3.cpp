...
{
    // load images
    vector<cv::Mat> imgList; //We want to flexibly rearrange data as well as delete and 
    //append elements on a regular basis. In C++, this can easily be achieved by using vectors. 
    //In the following code, a set of images is loaded from file as before and pushed into a dynamic list of type vector
    for (int i = 5; i <= 9; i++)
    {
        // create file name

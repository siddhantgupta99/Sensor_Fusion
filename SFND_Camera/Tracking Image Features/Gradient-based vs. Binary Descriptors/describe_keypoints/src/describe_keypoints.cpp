...
void descKeypoints1()
{
    ...
    detector1->detect(imgGray, kptsSIFT);
    t1 = ((double)cv::getTickCount() - t1) / cv::getTickFrequency();
    cout << "SIFT detector with n= " << kptsSIFT.size() << " keypoints in " << 1000 * t1 / 1.0 << " ms" << endl;

    cv::Ptr<cv::DescriptorExtractor> descriptor1 = cv::SIFT::create();
    cv::Mat descSIFT;
    t1 = (double)cv::getTickCount();
    descriptor1->compute(imgGray, kptsSIFT, descSIFT);
    t1 = ((double)cv::getTickCount() - t1) / cv::getTickFrequency();
    cout << "SIFT descriptor in " << 1000 * t1 / 1.0 << " ms" << endl;

    // visualize results
    cv::Mat visImage1 = img.clone();
    cv::drawKeypoints(img, kptsSIFT, visImage1, cv::Scalar::all(-1), cv::DrawMatchesFlags::DRAW_RICH_KEYPOINTS);

...
cv::Mat PerformNMS(cv::Mat corner_img)
{
    ...
    for (int r = sw_dist; r < corner_img.rows - sw_dist ; r++) // rows
    {
        //std::cout << "Corner Image rows" << r << std::endl;
        for (int c = sw_dist; c < corner_img.cols - sw_dist ; c++) // cols
        {
            // loop over all pixels within sliding window around the current pixel
           // std::cout << "Corner Image cloumns" << c << std::endl;
            unsigned int max_val{0}; // keeps track of strongest response // {0} : means that we have initialized the max_val to 0.
